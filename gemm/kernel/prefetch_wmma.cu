...
__global__ void pre_wmma::gpuWmmaPrefetchSgemm(int m, int n, int k,
  ...
  for (int i = 0; i < elePerThreadA; i += 1) {
    ...
    regA[i] = A[(row + bx * BM) * k + column];
  }
  for (int i = 0; i < elePerThreadB; i += 1) {
    int row = (i * tnum + tid) / BK;
    int column = (i * tnum + tid) % BK;
    regB[i] = B[(row + by * BN) * k + column];
  }
  ...
  for (int ko = 1; ko < k / BK; ko += 1) {
    ...
    for (int i = 0; i < elePerThreadA; i += 1) {
      int row = (i * tnum + tid) / BK;
      int column = (i * tnum + tid) % BK;
      regA[i] = A[(row + bx * BM) * k + (column + ko * BK)];
    }
    for (int i = 0; i < elePerThreadB; i += 1) {
      int row = (i * tnum + tid) / BK;
      int column = (i * tnum + tid) % BK;
      regB[i] = B[(row + by * BN) * k + (column + ko * BK)];
    }
