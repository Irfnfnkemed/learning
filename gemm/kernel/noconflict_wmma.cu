...
__global__ void noconflict_wmma::gpuNoConflictWmmaSgemm(int m, int n, int k,
                                                        const half *A,
                                                        const half *B, half *O,
                                                        const float alpha,
                                                        const float beta) {
  ...
  for (int ko = 0; ko < k / BK; ko += 1) {
    ...
    for (int i = 0; i < elePerThreadA / 2; i += 1) {
      ...
      SA[(row / 16 * (BK / 16 * 16 * 16)) + (column / 16 * (16 * 16)) +
         ((row % 16) * 16) + (column % 16) + 1] =
          A[(row + bx * BM) * k + (column + ko * BK) + 1];
    }
